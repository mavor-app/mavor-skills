---
name: shopify-subscription-mrr-tracker
displayName: Subscription Mrr Tracker
description: >-
  Read-only: for stores with subscription products, calculates MRR, ARR, active
  subscriber count, and rolling churn rate from subscription contracts.
version: 1.0.0
category: finance
runtime:
  type: llm
...
input:
  churn_window_days:
    ...
    required: false
    description: >-
      Rolling window for churn calculation (cancellations / starting
      subscribers)
  as_of:
    type: string
    required: false
