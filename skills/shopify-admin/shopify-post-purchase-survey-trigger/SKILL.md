---
name: shopify-post-purchase-survey-trigger
displayName: Post Purchase Survey Trigger
description: >-
  Read-only: identifies orders 7–14 days post-fulfillment that are eligible for
  a post-purchase survey campaign, excluding refunded or cancelled orders.
version: 1.0.0
...
input:
  survey_min_days:
    ...
    required: false
    description: >-
      Earliest days after fulfillment to survey (give time for delivery +
      initial use)
  survey_max_days:
    type: number
    required: false
  ...
  exclude_repeat_recipients_days:
    ...
    required: false
    description: >-
      Skip customers who were already on a survey list within this window
      (caller-tracked)
  format:
    type: string
    required: false
